...
public class ContactPanel extends JPanel {
  private final ContactController ctrl;
  private JTable table; private DefaultTableModel mdl;

  public ContactPanel(int sid){ctrl=new ContactController(sid);init();reload();}
  private void reload(){
    mdl.setRowCount(0);
    for(var c:ctrl.getAll())
      mdl.addRow(new Object[]{c.getContactId(),c.getFullName(),c.getRole(),c.getCategory(),c.getEmail(),c.getPhone(),c.getOrganization(),c.getNotes()});
  }
  private void showAdd(){
    JTextField na=f(),ro=f(),em=f(),ph=f(),or=f(),no=f();
    JComboBox<String> ca=combo("DSO Advisor","Academic Advisor","Professor","Embassy","Healthcare","Legal","Other");
    if(dlg("Add Contact","Full Name:",na,"Role:",ro,"Category:",ca,"Email:",em,"Phone:",ph,"Organization:",or,"Notes:",no)){
      if(ctrl.add(na.getText().trim(),ro.getText().trim(),(String)ca.getSelectedItem(),em.getText().trim(),ph.getText().trim(),or.getText().trim(),no.getText().trim())){reload();ok("Added!");}else err("Name and category required.");
    }
  }
  private void showEdit(){
    int row=sel();if(row<0)return; int id=(int)mdl.getValueAt(row,0);
    JTextField na=f(s(row,1)),ro=f(s(row,2)),em=f(s(row,4)),ph=f(s(row,5)),or=f(s(row,6)),no=f(s(row,7));
    JComboBox<String> ca=combo("DSO Advisor","Academic Advisor","Professor","Embassy","Healthcare","Legal","Other"); ca.setSelectedItem(s(row,3));
    if(dlg("Edit Contact","Full Name:",na,"Role:",ro,"Category:",ca,"Email:",em,"Phone:",ph,"Organization:",or,"Notes:",no)){
      if(ctrl.update(id,na.getText().trim(),ro.getText().trim(),(String)ca.getSelectedItem(),em.getText().trim(),ph.getText().trim(),or.getText().trim(),no.getText().trim())){reload();ok("Updated!");}else err("Update failed.");
    }
  }
  private void init(){
    setLayout(new BorderLayout());setBackground(BG);setBorder(BorderFactory.createEmptyBorder(24,24,24,24));
    mdl=new DefaultTableModel(new String[]{"ID","Name","Role","Category","Email","Phone","Organization","Notes"},0){public boolean isCellEditable(int r,int c){return false;}};
    table=makeTable(mdl);hideCol(0);
    add(hdr("Contacts",e->showAdd()),BorderLayout.NORTH);
